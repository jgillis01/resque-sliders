    ...
    module ResqueSliders
      ...
      class KEWatcher
        ...
        def initialize(options={})
          ...
          @verbosity = config[:verbosity].to_i # verbosity level
          @zombie_term_wait = config[:zombie_term_wait] # time to wait before TERM
          @zombie_kill_wait = set_zombie_kill_wait(config) # time to wait before -9
          @hostile_takeover = config[:force] # kill running kewatcher?
          @rakefile = config[:rakefile]
          @pidfile = config[:pidfile]
          @pidfile = set_pidfile(config[:pidfile])
          ...
          @async = config[:async] # sync and wait by default
          @hupped = 0
          Resque.redis = determine_redis_from_config(config)
        end
        ...
        def set_pidfile(pidfile)
           pidfile =~ /\.pid$/ ? pidfile : pidfile + '.pid' if pidfile
        end
        ...
        def determine_redis_from_config(options)
          case options[:config]
            when Hash
              [
                options[:config]['host'],
        ...
        def set_zombie_kill_wait(config)
          timeout = Integer(ENV.fetch('RESQUE_TERM_TIMEOUT') { 0 })
          zombie_kill_wait = timeout + @zombie_term_wait
          zombie_kill_wait ||= config[:zombie_kill_wait]
        end
